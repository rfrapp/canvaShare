...
void draw()
{
    trigonRGBA(renderer, (current.x2 + current.x1) / 2, (current.y2 + current.y1) / 2, current.x2, current.y2, 
               current.x1, current.y2, 0xFF, 0x00, 0x00, 0xFF);

    for (int i = 0; i < rects.size(); i++)
    {
        trigonRGBA(renderer, (rects[i].x2 + rects[i].x1) / 2, (rects[i].y2 + rects[i].y1) / 2, rects[i].x2, rects[i].y2, 
               rects[i].x1, rects[i].y2, 0xFF, 0x00, 0x00, 0xFF);
    }
}
