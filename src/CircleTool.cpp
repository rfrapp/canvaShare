...
CircleTool::CircleTool(Canvas *c, Rect r)
              : Tool(c, r), clicked(false),
                pointer_width(5), item(NULL)
{
}

void CircleTool::handle_input(SDL_Event *e)
{
	if (active)
	{
		int x, y;
...
void CircleTool::draw(SDL_Renderer *r)
{
	int x, y;
	SDL_GetMouseState(&x, &y);
...
void CircleTool::toggle_activate()
{
